...
-- A prior SQL Editor run may have stopped at the auth-schema grant. Replaying
-- this file must preserve policies already created by a successful run.
do $$ begin
  if not exists (select 1 from pg_policies where schemaname = 'public'
      and tablename = 'workspaces' and policyname = 'Runtime members view workspaces') then
  ...
  if not exists (select 1 from pg_policies where schemaname = 'public'
      and tablename = 'workspace_members' and policyname = 'Runtime members view memberships') then
  ...
  if not exists (select 1 from pg_policies where schemaname = 'public'
      and tablename = 'analysis_runs' and policyname = 'Runtime members view analysis') then
  ...
  if not exists (select 1 from pg_policies where schemaname = 'public'
      and tablename = 'graph_checkpoints' and policyname = 'Runtime members view checkpoints') then
  ...
  if not exists (select 1 from pg_policies where schemaname = 'public'
      and tablename = 'graph_checkpoints' and policyname = 'Runtime members write checkpoints') then
  ...
  if not exists (select 1 from pg_policies where schemaname = 'public'
      and tablename = 'graph_checkpoints' and policyname = 'Runtime members update checkpoints') then
